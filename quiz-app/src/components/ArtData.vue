<template>
  <div>
    ...
    <!-- Display count from Vuex store -->
    <h3>Count from Store: {{ count }}</h3>
  </div>
</template>
...
export default {
  ...
  computed: {
    // Compute the current question based on index
    currentQuestion() {
    ...
    // Get the correct count from the Vuex store
    count() {
      return this.$store.getters.getCount; // Using Vuex getter to get the correct answers count
    }
  },
  methods: {
    ...
    checkAnswer(selectedAnswer, correctAnswer) {
      if (selectedAnswer === correctAnswer) {
        this.$store.dispatch('increment'); // Dispatch the 'increment' action to update the store
        this.score++; // Increment the local score if the answer is correct
      }
